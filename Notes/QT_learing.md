...
![image-20230321150557415](图片\image-20230321150557415.png)
...
![image-20230328113244566](图片\image-20230328113244566.png) 
...
![image-20230411143956150](图片\image-20230411143956150.png)
...
![image-20230411151209410](图片\image-20230411151209410.png)
...
![image-20230411154353556](图片\image-20230411154353556.png)
...
接收存在乱码的情况。![image-20230528164029133](图片\image-20230528164029133.png)
...
2）新建仓库：登录你的 GitHub 账户后，点击页面右上角的加号图标，选择 "New repository" 来创建一个新的仓库。给仓库取一个名字，选择仓库的公开或私有性质，并填写其他相关信息，然后点击 "Create repository" 创建仓库。

3）克隆仓库（Clone）：在你创建的仓库页面中，点击绿色的 "Code" 按钮，复制仓库的 URL。打开本地你已经准备好的文件夹，然后，在该文件夹路径上打开命令行终端（window上面就是git bash或者powershell），使用 `git clone` 命令将仓库克隆到本地。例如：
...
4）添加和提交文件：在你克隆的本地仓库文件夹中，你可以添加、修改和删除文件。在你完成对文件的修改后，使用以下命令来提交更改：

```C++
git add .//将所有修改的文件添加到暂存区
git commit -m "提交说明"//将暂存区的内容提交到本地仓库。请替换 "提交说明" 为你对本次提交的简短描述。
```

5)推送到服务器：旦你已经提交了更改，你可以使用以下命令将本地仓库中的更改推送到 GitHub：

```c++
git push origin main
```

然后每次上传代码的时候（多人合作，个人在不同电脑上写）

先从服务器拉代码到本地，更新别人/自己之前写的代码：
...
1. 按下键盘上的 "i" 键，进入编辑模式。
2. 输入你的合并提交消息，描述为什么该合并是必要的。可以提供任何有意义的描述，以便记录合并的目的。
3. 按下键盘上的 "Esc" 键，退出编辑模式。
4. 输入 `:wq`，然后按下回车键，保存并退出编辑器。

### 3)push ssh方式

remote: Support for password authentication was removed on August 13, 2021.
remote: Please see https://docs.github.com/en/get-started/getting-started-with-git/about-remote-repositories#cloning-with-https-urls for information on currently recommended modes of authentication.
fatal: Authentication failed for 'https://github.com/AVAkingslayer/qt_c.git/'

现在已经不能用https的方式进行代码上传，可以使用SSH：

每台电脑上生成的密钥对都是独立的，所以你需要在**每台电脑上重复上述步骤来配置 SSH 密钥。****

配置并使用 SSH 密钥进行身份验证。这样可以避免使用用户名和密码进行身份验证。你可以在 GitHub 上生成 SSH 密钥，并将公钥添加到你的 GitHub 帐户中。然后，将远程仓库的 URL 修改为使用 SSH 协议。例如：`git remote set-url origin git@github.com:AVAkingslayer/qt_c.git`

1）首先在 GitHub 上生成 SSH 密钥，并将公钥添加到你的 GitHub 帐户中。

ssh-keygen -t rsa -b 4096 -C "565228404@qq.com"

生成 SSH 密钥对：如果你在其他电脑上没有生成 SSH 密钥对，请在该电脑上打开终端或命令行界面，并执行以下命令来生成 SSH 密钥对：

```c++
ssh-keygen -t rsa -b 4096 -C "your_email@example.com"
```

这将生成一对 SSH 密钥，包括公钥和私钥。在生成过程中，你可以选择自定义文件路径和密钥密码，或者使用默认值。

复制公钥：打开生成的公钥文件（默认为 `id_rsa.pub`），复制其中的内容。

添加公钥到 GitHub 帐户：在其他电脑上，前往 GitHub 网站，登录到你的帐户，打开你的个人设置，在 "SSH and GPG keys"（SSH 和 GPG 密钥）选项中点击 "New SSH key"（新建 SSH 密钥），将之前复制的公钥粘贴到 "Key" 字段中，并为该密钥提供一个可识别的标题。

配置本地仓库：在其他电脑上的本地仓库中，将远程仓库的 URL 修改为使用 SSH 协议。执行以下命令将远程仓库的 URL 修改为使用 SSH 协议：

1. ```
	git remote set-url origin git@github.com:AVAkingslayer/qt_c.git
	```

	这将将远程仓库的 URL 修改为使用 SSH 协议，以便使用 SSH 密钥进行身份验证。

现在，在其他电脑上你应该可以使用 SSH 密钥进行身份验证，并上传代码到 GitHub 仓库了。确保将私钥（`id_rsa`）妥善保管，并不与他人共享。每台电脑上生成的密钥对都是独立的，所以你需要在每台电脑上重复上述步骤来配置 SSH 密钥。(C:\Users\Administrator\.ssh)
...
```
git config --global http.proxy socks5://122.58.86.37:2801
```
...
![](图片\image-20230413152720645.png)

64位的git

![image-20230413152756756](图片\image-20230413152756756.png)

②然后在想要的文件夹里面创建仓库，右键然后Git create repository here

然后就出现了一个空的git文件夹![image-20230413153222765](图片\image-20230413153222765.png)
...
![image-20230413153300829](图片\image-20230413153300829.png)

每次上传代码的时候要点一下pull，更新文件夹，把文件复制到该仓库问价夹上

![image-20230413153640791](图片\image-20230413153640791.png)

然后点commit，输入版本信息

![image-20230413153810823](图片\image-20230413153810823.png)
...
​		在“\dso_demo”文件夹中打开命令行，输入“git gui&”。

#### 3）拉取代码

直接gitpull或者！377（执行第377行的代码） 一定要先拉取代码！！！

​		从服务器拉取代码，按下图步骤进行操作

![git代码拉取](图片/git代码拉取.png)

#### 4）分支合并

​		好像在linux上面的git分支管理有一定的问题，拉取代码之后不能直接改变对应文件，需要进行分支合并。在linux上面修改代码并不方便，一般不在linux上直接修改代码，为了分支管理方便，直接选择**硬性：丢弃本地修改**，将本地分支版本与服务器分支版本统一。

​		按下图步骤操作：

#### ![git版本合并](图片/git版本合并.png)

​		提交本地修改，按下图步骤操作。

![git代码提交](图片/git代码提交.png)

#### 5）编译时间检测问题

​		我们修改了系统时间，导致服务器拉取下来的代码，与系统时间不一致，有时候可能会发生“make”编译代码时，检测不到代码改变。

​		两种解决方法：

​		1）输入“make clean”清理编译问题，再“make”重新编译。

​		2）进入“makefile“文件所在的文件夹，手动删除对应*.o文件，再“make”重新编译。

#### 6）测试工程路径

1.mount

2.cd /media/data/ 进入工程下面，并且显示有哪些文件夹
...
ip a 获得该台电脑的地址，那么这台电脑的地址为inet 192.168.2.143/24

先克隆一台新的文件夹在上面 

git clone http://192.168.2.2/dso_demo.git dso_demo2

再创建共享文件夹

安装文件比较软件，用于比较两者的区别：

![image-20230613112742753](../../AppData/Roaming/Typora/typora-user-images/image-20230613112742753.png)

8)git查看历史版本，并且获取该历史版本下的代码

https://blog.csdn.net/qq_38394518/article/details/121394169

### 2.qt qmake

（等待补充）

.pro文件中的qmake–>makefile—>obj—>.so

qt中的ui文件最后会封装成.cpp和.h文件

用QtCreator通过.pro文件打开工程，结果编译的时候发生错误如下图所示：根本原因是通过vs生成的pro配置文件是有问题的，虽然可以修改，但是比较麻烦，特别是对于新手看不懂pro内容的同学来说。所以如果是想要把vs的Qt项目移植到linux下，最好不要通过vs生成pro文件。只有当拷贝到linux下时才通过qmake -project命令生成pro文件。
...
https://mp.weixin.qq.com/s?__biz=MzI3MjY1ODI2Ng==&mid=2247485996&idx=1&sn=0a08bf2f04eed49542a96cee6928a561&chksm=eb2e7019dc59f90f55b0aaabf7a59a16a4e06bf941beda82e2e61e807da3ca8d4c2e247173ad&scene=27

![image-20230612140909794](../../AppData/Roaming/Typora/typora-user-images/image-20230612140909794.png)

当运行不成功的时候不要忘记配置run–>command line arguments

*可以将一些值从命令行传递给Objective-C程序*。 这些值称为command line arguments,很多时候它们对您的程序很重要,特别是当您想从外部控制程序而不是在代码中对这些值进行硬编码时。

### 3.ShutDownDlg for linux

直接在linux的qt下面写一个一运行就关掉虚拟机的代码，记得用sudo-v权限

如果是在命令行关机的话，输入：

```c++
sudo shutdown -P now//告诉unbutu立即关机
//可以把now换成+m，m表示的是想在多长时间后进行关机的数字
//并且需要提供管理员权限                 
```

一定要选择复制虚拟机，否则的话会出问题。关闭后关掉电源。

```c++
QString shutdownCommand = "sudo shutdown -h now";
QProcess process;
process.start(shutdownCommand);
process.waitForFinished();
```

这样写没有用，开始starting以后并没有关机，尝试用system进行解决：

```c++
system("echo 's'|sudo -S shutdown -h now");

QProcess *sysclkprocess = new QProcess();//method2
    QStringList arguments;
    arguments << "-S" << "shutdown" << "-h" << "now";
    sysclkprocess->start("sudo", arguments);
    QString password = "s";
    sysclkprocess->write(password.toUtf8() + "\n");
```

这种方式会导致虚拟机非法关机，最好使用虚拟化软件提供的正常方式关机，例如vmrun

git 怎么样找到历史的程序？

### 4 .SetSystemTime(&st) 系统时间设置
...
（3）QProcess *process = new QProcess();

process->start("ls");
```

https://blog.csdn.net/ken2232/article/details/129218001

1、以上3中方法都可以，但是前两种会阻塞进程，直到ls结束，才会进行后续的操作，而第三种不会阻塞进程，可以多任务运行。

2、当命令中存在**管道（|）和重定向（>）**时，只能使用第二种，第一种和第三种属于Qt自身的，不能识别这两种符号。

#### 1).首先实现用命令行设置系统时间。

Ubuntu时钟分为系统时钟（System Clock）和硬件（Real Time Clock，简称RTC）时钟

ubuntu修改系统时间，查看系统时间：date 

2023年 06月 13日 星期二 15:11:04 CST

查看硬件时间：RTC

当你启动Ubuntu时，操作系统会读取硬件时钟的值，并加载为系统时间。然后，系统时钟开始计时，跟踪时间的流逝。

系统时钟在运行时可以被调整，以确保时间的准确性。例如，Ubuntu可以通过与网络时间协议（NTP）服务器同步，使系统时间与全球的标准时间保持一致。

相当于是硬件时钟和系统时钟一起工作来确保计算机的准确性。
...
也就是输入sodu  hwclock -r就能显示硬件时间 2023-06-13 15:25:16.993561+08:00

2023年 06月 13日 星期二 15:11:04 CST(标准时间)

在Ubuntu中，**硬件时钟（RTC）通常由计算机的BIOS或UEFI管理，并且无法直接通过命令行进行修改**。硬件时钟的设置在操作系统级别是受限制的，因此需要通过BIOS或UEFI界面来进行修改。

然而，可以通过命令行来设置系统时间，而不是直接修改硬件时钟。通过修改系统时间，当计算机重新启动时，系统会将当前系统时间加载到硬件时钟中。

```c++
sudo date -s "2023-06-14 12:34:56"
2023年 06月 13日 星期二 15:11:04 CST
 sudo date -s "YYYY-MM-DD HH:MM:SS"
```

首先需要关掉时间同步服务：Ubuntu通常会启用时间同步服务（如systemd-timesyncd）来自动同步系统时间。这些服务会定期从NTP服务器获取时间并进行调整。

```c++
sudo systemctl stop systemd-timesyncd.service
```

关掉了时钟同步以后，时钟被修改成功。但是重启以后还是恢复初始设置。
...
//后面要启动就用 enable start ture就行了
```

确实是ban掉了，但是重启还是恢复了原始时钟

怎么退出文本模式：ctrl+c

再检查其他可能影响时钟同步的地方：

![image-20230613174242455](图片/image-20230613174242455.png)

再检查下需要同步到硬件时钟，重启后时间就已经被修改了

```bash
sudo date -s "2023-06-14 17:47:00"
sudo hwclock --systohc//同步到硬件时钟
```

但是由于禁掉了ntp服务，打开QT后虚拟机就进入黑屏死机的状态：

![image-20230614110216174](图片/image-20230614110216174.png)

后面重新复制的虚拟机，并且pull代码，还是很方便的，下次遇到问题可以这么搞。
...
a)避免不必要的拷贝：使用原始对象的引用。这样可以节省内存和时间，并提高函数的执行效率。b）保护参数：**函数内部不能对参数进行修改****

#### 3).归一化

由于获取外部时间有cfg,gps,ntp几种方式，所以还要考虑如何归一化这几个参数，其中最大的区别是UTC/Local的区别，需要百度一下

4)系统崩溃，错误日志查询

ubuntu rfkill:input handler enabled

通过Ctrl+AlT+F3 切换到tty3 命令行登录正常.
 通过命令行 dmesg命令查看日志有如下报错:

![image-20230614112714339](图片/image-20230614112714339.png)
